...
class Visage: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    init(cameraPosition : CameraDevice, optimizeFor : DetectorAccuracy) {
        ...
        switch optimizeFor {
        case .batterySaving : faceDetectorOptions = [CIDetectorTracking: true as AnyObject, CIDetectorAccuracy : CIDetectorAccuracyLow as AnyObject]
        case .higherPerformance : faceDetectorOptions = [CIDetectorTracking: true as AnyObject, CIDetectorAccuracy : CIDetectorAccuracyHigh as AnyObject]
        }
    ...
    func captureOutput(_ captureOutput: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        if (features.count != 0) {
            ...
            for feature in features as! [CIFaceFeature] {
                ...
                let realX = uiBounds.width - faceBoundsFromImage.origin.y * uiBounds.width / sourceImage.extent.size.height - realWidth
                let realY = faceBoundsFromImage.origin.x * uiBounds.height / sourceImage.extent.size.width
                faceBounds = CGRect(x: realX, y: realY, width: realWidth, height: realHeight)
                ...
                print(NSDate().timeIntervalSinceReferenceDate, feature.leftEyeClosed, feature.rightEyeClosed)
            }
        } else {
            if (onlyFireNotificatonOnStatusChange == true) {
